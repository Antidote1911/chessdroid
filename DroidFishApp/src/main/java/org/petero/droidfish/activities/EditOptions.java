...
public class EditOptions extends AppCompatActivity {
    ...
    private void initUI() {
        ...
        // Make sure DecorView doesn't fit system windows so we get insets
androidx.core.view.WindowCompat.setDecorFitsSystemWindows(getWindow(), false);

// Views we'll adjust (declare final so they can be used inside the lambda)
final View contentView = findViewById(R.id.eo_content);
final View okBtn = findViewById(R.id.eo_ok);
final View cancelBtn = findViewById(R.id.eo_cancel);
final View resetBtn = findViewById(R.id.eo_reset);

// Parent of buttons (LinearLayout containing the three buttons), computed once
final View buttonBar = (okBtn != null && okBtn.getParent() instanceof View) ? (View) okBtn.getParent() : null;

// Install the listener on the root so we get both top and bottom insets
final View root = findViewById(android.R.id.content);
androidx.core.view.ViewCompat.setOnApplyWindowInsetsListener(root, (v, windowInsets) -> {
    androidx.core.graphics.Insets sysInsets =
        windowInsets.getInsets(androidx.core.view.WindowInsetsCompat.Type.systemBars());

    // Compute action bar height
    ...
    // Apply top padding to content (status bar + action bar)
    if (contentView != null) {
        contentView.setPadding(sysInsets.left,
                               sysInsets.top + actionBarHeight,
                               sysInsets.right,
                               contentView.getPaddingBottom()); // keep existing bottom padding
    }

    // Apply bottom padding to the button bar so buttons sit above nav bar
    if (buttonBar != null) {
        buttonBar.setPadding(buttonBar.getPaddingLeft(),
